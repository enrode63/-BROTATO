...
- 플레이어 이동(WASD·방향키) + **자동 조준/자동 발사**, 무기 아이콘이 캐릭터 주위를 공전
- 무기 4종: **카메라**(샷건) · **시운이의 커터칼**(근접) · **우람한 오이**(관통 창) · **트페의 카드**(골드=스턴/블루=경험치/레드=폭발)
- 투척물 3종(1/2/3키, 마우스 조준): 수류탄(폭발)·섬광탄(화면 흰섬광+전체 3초 속박)·화염병(지속 화염)
- 상점 5칸(무기2·투척물1·능력치2): 능력치(삼두근·다리·심장·척추·이빨흡혈·쌀숭이골드)·체력회복·무기/투척물 구매·재굴림
- 무기 합성(인벤토리 드래그): 같은 무기 2개 → 레벨업(로마숫자 I~V, 레벨당 +50%)
- 게임오버: 도달 웨이브·처치 수·보스 처치 표시, R키로 시작 화면 복귀
